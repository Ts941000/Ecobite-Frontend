    ...
    return `
      <div class="ordercard" id="order-${order.rawId}">
        <div>
          ...
          <div class="oname">${order.name}</div>
          <div class="ometa">${order.user} \u2022 Qty: ${order.qty}</div>
          ${prepHtml}
        </div>
...
async function saveListing() {
  ...
  const checks = {
    ...
    "ferr-type": type === "",
    "ferr-time": time === "",
    "ferr-image": !imageFile && !editId && !editImageUrl,
  };
...
document.getElementById("orderlist").addEventListener("click", (event) => {
  ...
  if (acceptBtn) acceptOrder(acceptBtn.dataset.acceptId);
  if (cancelBtn) cancelOrder(cancelBtn.dataset.cancelId);
  if (confirmAcceptBtn) confirmAcceptOrder(confirmAcceptBtn.dataset.confirmAccept);
  if (confirmCancelBtn) confirmCancelOrder(confirmCancelBtn.dataset.confirmCancel);
