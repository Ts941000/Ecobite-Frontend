...
const currentUser = await requireAuth("login.html", ["admin", "hotel"]);

function escapeHtml(str) {
  return String(str ?? '').replace(/[&<>"']/g, c => ({ '&': '&amp;', '<': '&lt;', '>': '&gt;', '"': '&quot;', "'": '&#039;' }[c]));
}

// ── AGGRESSIVE CACHE-BUSTING CLEANUP ──
// If the browser served an old cached HTML file, force-remove the old elements.
try {
  const oldCards = document.getElementById("cards");
  if (oldCards) oldCards.remove();
  
  const oldLogos = document.querySelectorAll('a[href="EcoBite.html"]');
  oldLogos.forEach(link => {
    // If it's the dashboard logo, point it back to dashboard
    if (link.id === 'slogo' || link.closest('#mobileHeader')) {
...
// Hotel name will be loaded from Firestore profile after auth
async function loadHotelProfile() {
  if (!currentUser?.uid) return;
  try {
    const profile = await readUserProfile(currentUser.uid);
    if (profile && profile.hotelName) {
      document.getElementById('settingHotelName').value = profile.hotelName;
      ...
      document.getElementById('hotelnameText').textContent = profile.hotelName;
    }
  } catch (e) {
...
document.getElementById("saveSettingsBtn").addEventListener("click", async () => {
  ...
  // Persist to Firestore user profile (primary) and localStorage (backup)
  try {
    if (currentUser?.uid) {
      await writeUserProfile(currentUser.uid, { hotelName: hotel });
    }
  } catch (e) {
    console.error('Failed to save hotel name to Firestore:', e);
  }
  localStorage.setItem('ecobite-hotel-settings', JSON.stringify({ hotel }));
  showToast("Hotel name saved successfully");
});

// ── Real-time listeners (filtered by current admin's ownerId) ──

await listenListingsByOwner(currentUser?.uid, (items) => {
  listings = items || [];
  try { renderRows(); } catch (e) { console.error('renderRows error:', e); }
  try { renderActivity(); } catch (e) { console.error('renderActivity error:', e); }
}, []);

await listenOrdersByOwner(currentUser?.uid, (items) => {
  orders = items || [];
