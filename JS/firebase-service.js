...
export async function listenListingsByOwner(ownerId, callback, fallback = []) {
  if (!isFirebaseConfigured()) {
    const all = getLocalListings(fallback);
    callback(all.filter(l => l.ownerId === ownerId));
    return () => {};
  }

  await initFirebase();
  const { collection, onSnapshot, orderBy, query, where } = sdk.firestoreModule;
  return onSnapshot(
    query(
      collection(db, firebaseCollections.listings),
      where("ownerId", "==", ownerId)
    ),
    (snap) => {
      const items = snap.docs.map((entry) => normalizeListing(entry.id, entry.data()));
      items.sort((a, b) => (b.createdAtMs || 0) - (a.createdAtMs || 0));
      callback(items);
    },
    (error) => {
      console.error("Owner listing subscription failed:", error);
      callback(fallback);
    }
  );
}
...
export async function listenOrdersByOwner(ownerId, callback) {
  if (!isFirebaseConfigured()) {
    ...
    callback(all.filter(o => (o.items || []).some(item => item.ownerId === ownerId)));
    return () => {};
  }
  ...
  return onSnapshot(
    query(collection(db, firebaseCollections.orders), orderBy("createdAtMs", "desc")),
    (snap) => {
      const all = snap.docs.map((entry) => ({ ...entry.data(), id: entry.id }));
      // Filter to only orders that contain items belonging to this admin
      const filtered = all.filter(order =>
        (order.items || []).some(item => item.ownerId === ownerId)
      );
      callback(filtered);
    },
    (error) => {
      console.error("Owner order subscription failed:", error);
      callback([]);
    }
  );
}
