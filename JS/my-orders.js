import { requireAuth, getSignedInUser, listenOrders, logoutUser, updateOrderStatus } from './firebase-service.js';
    ...
    return `
      <div class="order-card" style="animation-delay: ${i * 0.06}s">
        ...
        <div class="order-footer">
          ...
          ${status === 'pending'
            ? `<button class="cancel-order-btn" data-cancel-order="${order.id}">
                 <span class="material-symbols-rounded">cancel</span> Cancel Order
               </button>`
            : ''}
          ${status === 'accepted'
            ? `<button class="track-order-btn" data-track-order="${order.id}">
                 <span class="material-symbols-rounded">map</span> Track Order
               </button>`
            : ''}
        </div>
      </div>
...
function showToast(msg, icon = 'check_circle') {
  ...
  if (!toast) {
    ...
    toast.style.cssText = `
      ...
      background:linear-gradient(135deg,#1a2e1a,#2d9b5a); color:#fff;
      padding:14px 24px; border-radius:14px; display:flex; align-items:center;
      gap:10px; font-size:14px; font-weight:600; font-family:'Montserrat',sans-serif;
      box-shadow:0 8px 30px rgba(0,0,0,0.25); z-index:999;
  ...
  requestAnimationFrame(() => { toast.style.bottom = '28px'; });
  setTimeout(() => { toast.style.bottom = '-80px'; }, 2800);
}

// ── Cancel & Track click handlers ──
container.addEventListener('click', async (event) => {
  ...
  if (cancelBtn) {
    ...
    cancelBtn.innerHTML = '<span class="material-symbols-rounded">hourglass_top</span> Cancelling...';
    try {
      await updateOrderStatus(orderId, 'cancelled', { cancelledAt: Date.now(), cancelledBy: 'customer' });
      showToast('Order cancelled successfully', 'check_circle');
    } catch (error) {
      console.error('Cancel failed:', error);
      showToast('Could not cancel order. Please try again.', 'error');
      cancelBtn.disabled = false;
      cancelBtn.innerHTML = '<span class="material-symbols-rounded">cancel</span> Cancel Order';
    }
  }

  if (trackBtn) {
    const orderId = trackBtn.dataset.trackOrder;
